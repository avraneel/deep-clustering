...
train_data = datasets.MNIST(root='./data/MNIST_data/', train=True, transform=transforms.ToTensor(), download=True)

test_data = datasets.MNIST(root='./data/MNIST_data/', train=False, transform=transforms.ToTensor(), download=True)
...
latent_size         = 10
dec                 = DEC_exp(latent_size).to(device)
mse                 = nn.MSELoss()
...
def pre_train(dec):
      ...
      plt.savefig(fname='./results/pre_train_exp.png', format='png')

pre_train(dec)

# Saving and Loading

PATH = './saved_models/pre_trained_exp.pth'
...
def train(dec):
      ...
      plt.ylabel('Loss')
      plt.plot(train_loss)
      plt.savefig(fname='./results/train_exp.png', format='png')
...
PATH = './saved_models/trained_exp.pth'
torch.save(dec.state_dict(), PATH)
...
with open("./results/e.txt", "a") as o:
...
plt.savefig(fname='./results/TSNE_exp_' + str(n_train) + '_epochs.png', format='png')
plt.show()