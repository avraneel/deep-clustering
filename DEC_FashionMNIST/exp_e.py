...
test_data = datasets.FashionMNIST(root='./data/FashonMNIST_data/', train=False, transform=transforms.ToTensor(), download=True)
...
def pre_train(dec):
      ...
      plt.savefig(fname='./results/pre_train_exp_fashion.png', format='png')

pre_train(dec)

# Saving and Loading

PATH = './saved_models/pre_trained_exp_fashion.pth'
...
def train(dec):
      ...
      plt.ylabel('Loss')
      plt.plot(train_loss)
      plt.savefig(fname='./results/train_exp_fashion.png', format='png')
...
PATH = './saved_models/trained_exp_fashion.pth'
torch.save(dec.state_dict(), PATH)
...
with open("./results/e_fashion.txt", "a") as o:
...
plt.savefig(fname='./results/TSNE_exp_' + str(n_train) + '_epochs_fashion.png', format='png')
plt.show()
